...
function loadStatsPage()
{
	...
	{
		if(Number(table.rows[row].cells[2].innerHTML) > Number(table.rows[row].cells[3].innerHTML))
		{
...
function loadPlayersPage()
{
	players.forEach(function(item, index) {
		...
		a.href='#';															//edit anchor properties
		a.text = item.name;
		a.onclick = function() {switchPlayers(index)};


		var node = document.createElement("li"); //create list element and append anchor
		node.appendChild(a);

		document.getElementById("player_selector").appendChild(node); //append list element to drop down
	});
};
